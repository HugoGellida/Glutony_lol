    ...
    DocumentBuildMode mode) const
{
    ...
    const std::string styleAttribute = buildStyleAttribute({
        ...
        {"flex-grow", propertyValue(properties, "flex-grow", "0") == "0" ? "" : propertyValue(properties, "flex-grow", "0")},
        {"flex-shrink", propertyValue(properties, "flex-shrink", "1") == "1" ? "" : propertyValue(properties, "flex-shrink", "1")}
    });
...
{
    return ".widget_scroll_region { display: block; height: 140px; min-width: 0px; min-height: 0px; overflow-y: auto; overflow-x: hidden; padding: 12px; box-sizing: border-box; border: 1px #ccbfa9; background-color: #fff9ef; }\n";
}
