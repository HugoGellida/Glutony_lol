...
vec3 sampleAlbedoLinear(vec4 albedoTexel)
{
        return pow(albedoTexel.rgb, vec3(2.2)) * clamp(_mainCol, vec3(0.0), vec3(1.0));
}
...
void main()
{
        ...
        vec3 albedo = sampleAlbedoLinear(albedoTexel);
